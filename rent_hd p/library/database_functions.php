<?php
	function db_connect(){
		$conn = mysqli_connect("localhost", "root", "", "library_hdp");
		if(!$conn){
	...
	function select4LatestBook($conn){
		...
		if(!$result){
		    echo "Can't fetch ! " . mysqli_error($conn);
		    exit;
		}
		for($i = 0; $i < 4; $i++){
	...
	function insertIntoOrder($conn, $customerid, $total_price, $date, $ship_name, $ship_address, $ship_city, $ship_zip_code, $ship_country){
		...
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Order loading failed " . mysqli_error($conn);
			exit;
		}
	}
	...
	function getbookprice($isbn){
		...
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Fetch stationery failed! " . mysqli_error($conn);
			exit;
		}
		$row = mysqli_fetch_assoc($result);
	...
	function getPubName($conn, $pubid){
		...
		if(mysqli_num_rows($result) == 0){
			echo "Something went wrong! check again";
			exit;
		}
